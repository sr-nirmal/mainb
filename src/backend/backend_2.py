...
def get_lineitems():
    ...
    # print(line_items)
    line_items = []
    for i in currentLineItems[name]:
        if (i[0] == rec_name):
            line_items = i[1]
...
def get_reciepts():
    ...
    name = data["name"]
    print("name -> ", name)
    print("current_recipts -> ", currentReciptlist[name])
    return jsonify(recipt=currentReciptlist[name])
...
def get_reciepts_score():
    ...
    temp_list = currentReciptlist[name]
    print("current_recipts -> ", temp_list)
    ret = [int(i[1]) for i in temp_list]
    ratio = [0, 0, 0]
    for i in ret:
        if (i < 3):
            ratio[0] = ratio[0] + 1
...
def get_lineitems_score():
    ...
    date = []
    score = []
    for i in currentReciptlist[name]:
        date.append(i[2])
        score.append(i[1])
    d = {'date': date, 'score': score, 'ratio': ratio}
    print("date => ", date)
...
def write_line_items(name, file_name, line_items, collection, reciept_collection):
    ...
    reciept_collection.insert_one(
        {"name": name, "rec_name": recipt_name, "line_items": line_items, "score": calculate_score(line_items),
...
def start():
    ...
    try:
        if (name == tempData[0]):
            currentReciptlist[name] = tempData[1]
            currentLineItems[name] = tempData[2]
        else:
            load(name, collection, reciept_collection)
    except NameError:
        load(name, collection, reciept_collection)
...
def logout():
    data = request.get_json()
    name = data['name']
    currentLineItems.pop(name)
    currentReciptlist.pop(name)
    return jsonify({"resp":"success"})
...
def load(name, collection, recipt_collection):
    ...
    # try:
    #     if(currentLineItems != {} and currentReciptlist != {}):
    #         currentReciptlist = {}
    #         currentLineItems = {}
    # except NameError:
    #     print("executing....")
    ...
    # print(scores)
    # print(line_items)
    currentReciptlist[name] = []
    currentLineItems[name] = []
    for i in range(len(rec_names)):
        currentReciptlist[name].append([rec_names[i], int(scores[i]), date[i]])
        currentLineItems[name].append([rec_names[i], line_items[i]])
    print("Load function -> ", currentReciptlist)
    # for i in currentReciptlist.keys():
    #     print(currentReciptlist[i])
